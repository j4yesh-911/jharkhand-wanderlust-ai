...
const Itinerary = () => {
  ...
  const fetchRates = async () => {
    try {
      ...
      if (json && json.rates) setRates({ USD: json.rates.USD, EUR: json.rates.EUR });
    } catch (err) {
  ...
  const formatCurrencyUI = (amount: number) => {
    ...
    return new Intl.NumberFormat(locale, opts).format(Number(amount.toFixed(2)));
  };
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      - Custom Stops: ${aiPreferences.customStops.length > 0 ? aiPreferences.customStops.join(", ") : "None"}
      ...
      try {
        ...
        const cleanJson = jsonMatch[0].replace(/,(\s*[\]}])/g, "$1").replace(/\n/g, " ");
        ...
        parsed = parsed.map((d) => {
          const activities = d.activities.map((a) => {
            const cost = typeof a.estimatedCost === "number" && !isNaN(a.estimatedCost) ? a.estimatedCost : 0;
            return { ...a, estimatedCost: Math.round(cost) } as ItineraryActivity;
          });

          const dayTotal = typeof d.dayTotal === "number" && !isNaN(d.dayTotal)
      ...
      toast({
        title: "AI Generation Failed",
        description: "An error occurred with the Gemini API. Please try again later.",
        variant: "destructive",
      });
    } finally {
      setAiLoading(false);
  ...
  const handleSavePlan = () => {
    ...
    toast({ title: "Plan Saved", description: "This itinerary was added to your saved plans." });
  };

  const handleExport = () => {
    const blob = new Blob([JSON.stringify(aiSuggestions, null, 2)], { type: "application/json" });
  ...
  const openAllInMaps = () => {
    const allPlaces = aiSuggestions.flatMap((d) => d.activities.map((a) => a.name + ", Jharkhand"));
    window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(allPlaces.join("; "))}`, "_blank");
  };

  // compute totals for budget display
  const totalEstimatedINR = aiSuggestions.reduce((s, d) => s + (d.dayTotal || 0), 0);
  const remainingINR = aiPreferences.budget - totalEstimatedINR;
  const percentUsed = Math.min(100, Math.round((totalEstimatedINR / Math.max(1, aiPreferences.budget)) * 100));
      ...
      <div className="pt-16">
        <div className="bg-gradient-water py-20 text-white text-center">
          <div className="max-w-4xl mx-auto px-4">
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-5xl font-playfair font-bold mb-6">
              Plan Your Jharkhand Journey
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-xl">
        ...
        <div className="max-w-6xl mx-auto px-4 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-6 rounded-2xl space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {interests.map((interest) => (
                    <Button
                      key={interest}
                      size="sm"
                      variant={aiPreferences.interests.includes(interest) ? "default" : "outline"}
                    ...
                    <div key={i} className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setAiPreferences({ ...aiPreferences, customStops: newStops });
                        }}
                      />
                      <Button
                        variant="destructive"
                        ...
                        onClick={() =>
                          setAiPreferences({
                            ...aiPreferences,
                            customStops: aiPreferences.customStops.filter((_, idx) => idx !== i),
                          })
                        }
                      >
                        Remove
              ...
              <Button onClick={handleGenerateAI} className="btn-nature w-full" disabled={aiLoading}>
                {aiLoading ? (
                  <>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2 glass-card p-6 rounded-2xl space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold text-gradient-water">AI-Generated Itinerary</h3>
                <div className="flex gap-2 items-center">
                  <select value={currency} onChange={(e) => setCurrency(e.target.value as any)} className="border rounded p-1">
                    <option value="INR">INR</option>
                    <option value="USD">USD</option>
                    <option value="EUR">EUR</option>
                  </select>
                  <Button size="sm" variant="outline" onClick={handleSavePlan}>Save</Button>
                  <Button size="sm" variant="outline" onClick={handleExport}>Export</Button>
                  <Button size="sm" variant="outline" onClick={openAllInMaps}>Open All in Maps</Button>
                </div>
              </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center">
                  <div className="text-sm">Budget: {formatCurrencyUI(convert(aiPreferences.budget))} ({aiPreferences.budget} INR)</div>
                  <div className="text-xs text-muted-foreground">Estimated total: {formatCurrencyUI(convert(totalEstimatedINR))}</div>
                </div>

                <div className="mt-2 bg-gray-200 rounded h-3 overflow-hidden">
                  <div className="h-3" style={{ width: `${percentUsed}%`, background: percentUsed > 100 ? "#ef4444" : "#16a34a" }} />
                </div>
                <div className="text-xs mt-1">{formatCurrencyUI(convert(Math.max(0, remainingINR)))} remaining</div>
              </div>
                  ...
                  <Card key={index} className="p-4 bg-primary/5 border">
                    ...
                    <ul className="space-y-3">
                      {day.activities.map((activity, idx) => (
                        <li key={idx} className="flex justify-between items-center bg-white/50 rounded-lg p-3 shadow-sm">
                          <div className="flex-1">
                            <div className="flex items-center mb-1">
                              <Clock className="inline w-4 h-4 mr-2 text-primary" />
                              <span className="font-semibold">{activity.time}</span>
                              <span className="font-bold ml-2">{activity.name}</span>
                            </div>
                            {activity.description && <p className="text-sm text-muted-foreground ml-6">{activity.description}</p>}
                          </div>

                          <div className="ml-4 text-right">
                            <div className="text-sm font-medium">{formatCurrencyUI(convert(activity.estimatedCost || 0))}</div>
                            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(activity.name + ", Jharkhand")}`} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="text-sm font-semibold">Day total: {formatCurrencyUI(convert(day.dayTotal || 0))}</div>
                      <div className="text-xs text-muted-foreground">INR {day.dayTotal ?? 0}</div>
                    </div>
                  </Card>
                ))
              ) : (
                !aiLoading && (
                  <div className="text-sm text-muted-foreground">No itinerary yet. Select your preferences and click <strong>Generate AI Plan</strong>.</div>
                )
              )}
            </motion.div>
          </div>
